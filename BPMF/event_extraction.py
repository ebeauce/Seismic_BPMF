...
def align_traces(event, travel_times, stations,
                 time_before_P, duration):
    for s, sta in enumerate(stations):
        for tr in event.select(station=sta):
            ...
            tr.trim(starttime=starttime, endtime=endtime,
                    pad=True, fill_value=None)
    return event

def extract_event(origin_time, net, duration=60.,
    ...
    for s in range(n_stations):
        for c in range(n_components):
            filename = os.path.join(cfg.input_path, str(ot.year),
            ...
            try:
                tr = obs.read(file,
                              ...
                              endtime=ot+offset_start+duration+1)[0]
                tr.trim(starttime=ot+offset_start,
            ...
            S += tr
    t2 = give_time()
    print('{:.2f} s to extract the waveforms from {}.'.
            format(t2-t1, ot))
    return S

def extract_event_parallel(origin_time, net, duration=60.,
                           offset_start=0., folder='raw',
                           attach_response=False, verbose=True,
    ...
    ot = udt(float(int(ot.timestamp*cfg.sampling_rate))/cfg.sampling_rate)
    # path to data
    data_path = os.path.join(cfg.input_path, str(ot.year),
                             'continuous{:03d}'.format(ot.julday),
                             folder)
    target_starttime = ot + offset_start
    target_endtime = target_starttime + duration + 1.
    t1 = give_time()
    # get all names
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(load_data, f, target_starttime,
                                   target_endtime, attach_response,
                                   verbose, check_compression)\
                     for f in iter(files)]
...
def extract_event_realigned(origin_time, net, travel_times,
                            duration=20., offset_start=0.,
                            folder='raw', attach_response=False,
                            verbose=True, check_compression=False):
    """
    Event extractor: parallelized version.
    """
    import itertools
    ...
    target_starttimes = [(ot + offset_start + tts[s])
                         for s in range(len(tts))]
    target_endtimes = [(time + duration) for time in target_starttimes]
    # read all traces in parallel
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(load_data, *f, attach_response,
                                   verbose, check_compression)\
...
def generate_file_list(data_path, net, travel_times=None):
    ...
    if travel_times is not None:
        ...
        if travel_times.ndim == 1:
            ...
            travel_times = np.repeat(
                    travel_times, len(net.components)).reshape(
    ...
    for s in range(len(net.stations)):
        for c in range(len(net.components)):
            target_files = '*{}*{}*'.format(net.stations[s],
                                            net.components[c])
            new_files = glob.glob(os.path.join(data_path,
                                               target_files))
...
def preprocess_event(event, freqmin=None, freqmax=None,
    ...
    for tr in preprocessed_event:
        ...
        if sr_ratio > 1:
            tr.data = autodet.utils.lowpass_chebyshev_II(
                       tr.data, 0.49*target_SR, tr.stats.sampling_rate,
    ...
    if freqmin is None and freqmax is None:
        ...
        # bandpass filtering
        preprocessed_event.filter('bandpass', freqmin=freqmin,
