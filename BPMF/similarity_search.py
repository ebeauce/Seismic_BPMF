...
class MatchedFilter(object):
    ...
    """
    def __init__(self, template_group, min_channels=6, max_kurto=100.,
                 remove_edges=True, normalize=True,
                 max_CC_threshold=0.80, n_network_chunks=1,
                 threshold_type='rms', step=cfg.matched_filter_step,
                 max_memory=None, max_workers=None):
    ...
    def set_data(self, data):
        ...
        self.data_arr = self.data.get_np_array(
                self.template_group.stations,
                components=self.template_group.components)
        if self.normalize:
    ...
    def compute_cc_time_series(
        ...
        if tids is None:
            select_tts = np.arange(self.template_group.n_templates,
                    dtype=np.int32)
        else:
        ...
        if weight_type == 'simple':
            ...
            weights_arr /= norm
            # insufficient data
            invalid = (np.sum((weights_arr != 0.), axis=(1, 2))
                    < self.min_channels)
            weights_arr[invalid] = 0.
    ...
    def find_detections(self, minimum_interevent_time,
        ...
        with concurrent.futures.ThreadPoolExecutor(
                max_workers=min(len(tids), self.max_workers)) as executor:
            output = list(executor.map(
                self._find_detections_t, range(len(tids)), tids))
        detections.update(
                {output[i][1]: output[i][0] for i in range(len(output))})
        if verbose > 0:
            for tid in tids:
    ...
    def plot_cc(self, tid, ax=None, detection=None, figsize=(20, 7)):
        ...
        if tid not in self.cc.keys():
            print(f'{tid} is not in `self.cc.keys()`. Re-run'
        ...
        detection_threshold = time_dependent_threshold(
                cc_t, utils.sec_to_samp(self.threshold_window_dur, sr=sr),
                threshold_type=self.threshold_type)
        # saturate threshold as requested by the user
        detection_threshold = np.minimum(
                self.max_CC_threshold*np.sum(weights_t), detection_threshold)
        cc_idx = self.select_cc_indexes(cc_t, detection_threshold,
                utils.sec_to_samp(self.minimum_interevent_time, sr=sr)//step)

        time_indexes = np.arange(len(self.data.time),
                dtype=np.int32)[::step][:len(cc_t)]
        ax.plot(self.data.time[time_indexes], cc_t)
        ax.plot(self.data.time[time_indexes], detection_threshold, color='C3', ls='--',
                label='Detection Threshold')
        if len(cc_idx) > 0:
            ax.plot(self.data.time[cc_idx*step], cc_t[cc_idx*step],
        ...
        if detection is not None:
            ...
            ax.annotate('detection', (ot, detection.aux_data['cc']),
                        (ot + np.timedelta64(15, 'm'), min(ax.get_ylim()[1],
                            2.*detection.aux_data['cc'])),
    ...
    def plot_detection(self, detection, figsize=(20, 20),
        ...
        grid = fig.add_gridspec(nrows=len(detection.stations)+2,
                ncols=len(detection.components), hspace=0.35)
